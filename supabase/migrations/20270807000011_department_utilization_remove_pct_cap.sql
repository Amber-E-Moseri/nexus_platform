-- Remove the least(100, ...) cap on department utilization percentage.
--
-- The original RPC capped utilization at 100% via least(100, round(...)),
-- which flattens all overloaded departments to a uniform "100%" visual and
-- hides the magnitude of real overload. Example: ORS at 6.5 avg tasks/member
-- (capacity baseline 5 = 130%) and a hypothetical dept at 20 avg both render
-- as "100%", making it impossible to triage who needs help most.
--
-- Removing the cap lets the percentage scale linearly: departments within
-- capacity (≤5 per member) show 0–100%, departments overloaded show >100%
-- proportional to severity. The widget already displays the raw avg in text
-- form, so the percentage bar now acts as a visual ranking rather than a
-- capped binary "at/over capacity" flag.
