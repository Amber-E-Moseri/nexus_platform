-- Fix invitation RPC signature drift after group-space support.
-- The app now sends p_group_space_ids, but some databases still expose the
-- older 8-argument create_user_invitation RPC. Recreate the RPC using the
-- current invitation rules and add group-space persistence.
...
drop function if exists public.create_user_invitation(text, text, text, uuid, text, uuid, text, text);
...
declare
  ...
  v_space_id uuid;
begin
  v_user_id := new.accepted_user_id;
  ...
  if new.group_space_ids is not null and array_length(new.group_space_ids, 1) > 0 then
    foreach v_space_id in array new.group_space_ids loop
      ...
      values (v_space_id, v_user_id, 'member', new.invited_by)
