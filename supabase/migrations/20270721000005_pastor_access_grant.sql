...
--
-- Re-applied here under a fresh timestamp: this migration originally shipped
-- on main as 20270721000003_pastor_access_grant.sql, but that version number
-- collided with an unrelated fix pushed from a different branch, which left
-- the remote migration history believing version 20270721000003 was already
-- applied — silently skipping this one. Content unchanged; idempotent
-- either way (on conflict do nothing / create or replace function).
