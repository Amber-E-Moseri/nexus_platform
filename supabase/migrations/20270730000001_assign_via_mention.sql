-- RPC: assign user to task via @mention in comment, with rich context notification
-- Replaces the old "assign first mention to comment" pattern with proper task assignment
...
begin
  ...
  -- Check authorization: user must be able to assign (same as task_assignees_write policy)
  if not (
    v_task.created_by = auth.uid()
    or public.current_user_role() = any (array['super_admin', 'regional_secretary'])
    or public.has_space_role(auth.uid(), v_task.department_id, 'dept_lead')
  ) then
